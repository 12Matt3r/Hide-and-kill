...
// This class mirrors the logic and options of the user's HouseGen.js
// but only generates a serializable data structure, with no THREE.js dependencies.
class ServerHouseGen {
  constructor(seed) {
    ...
    this.worldData = { walls: [], floors: [], doors: [], hidingSpots: [], navPoints: [] };
  }
  ...
  generate() {
    ...
    for (let y = 0; y < floors; y++) {
      ...
      for (let i = 0; i < this.opts.roomsPerFloor; i++) {
        const col = i % 2;
        const row = Math.floor(i / 2);
        const room = { x: col * (this.opts.roomSize.x + 2), z: row * (this.opts.roomSize.y + 2), w: this.opts.roomSize.x, d: this.opts.roomSize.y, y: baseY };
        allRooms.push(room);
  ...
  _createWallOrDoor(room, side, wallHeight) {
    ...
    if (useDoor) {
      this.worldData.doors.push({ id: uuidv4(), p: pos, s: {w: 1, h: 2.2, d: 0.1}, r, isOpen: false, isLocked: false, isJammed: false, jamTimer: 0 });
    } else {
      this.worldData.walls.push({ id: uuidv4(), p: pos, s, isWeak: this.rng.next() < 0.2 });
    }
  }
